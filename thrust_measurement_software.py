...
plt.ion()
plt.figure(1)
phidget_bridge = PhidgetBridge(frequency) # Include phidgetbridge-related functions
...
with MultiThreadedGetChar() as getchar:
    for i in range (0, len(PWM_percentage_list), 1):    
        ...
        while time.time() < t_end:
            # Get force value from the load cell and add it to the local force array
            local_f_array.append(phidget_bridge.getForce())
            # Get voltage value from the arduino and add it to the local voltage array
            local_v_array.append(arduino_communication.getVoltageFromArduino())
            # If a characer is given, discontinue the loop and bring the motor to 0 PWM            
            if getchar():
        ...
        if not continue_looping:
            break
        p_array.append(percentage) # take current percentage and append to p_array
        local_f_np_array = np.array(local_f_array) # create an array from the local_f_array using numpy
        local_v_np_array = np.array(local_v_array) # create an array from the local_v_array using numpy
        avg_f_array.append(np.mean(local_f_np_array)) # take the average of the local force array and store in the averaged array
        std_f_array.append(np.std(local_f_np_array)) # take the average of the local std force array and store in the averaged array
        avg_v_array.append(np.mean(local_v_np_array)) # take the average of the local voltage array and store in the averaged array
        std_v_array.append(np.std(local_v_np_array)) # take the average of the local std voltage array and store in the averaged array
